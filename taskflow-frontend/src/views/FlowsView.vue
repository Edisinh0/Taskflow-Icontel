  ...
  <div class="min-h-screen bg-slate-50 dark:bg-slate-900 transition-colors">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div class="flex justify-between items-center mb-8">
        <div>
          <h2 class="text-3xl font-bold text-slate-800 dark:text-white tracking-tight">Flujos de Trabajo</h2>
          <p class="text-slate-500 dark:text-slate-400 mt-1 text-lg">Gestiona tus proyectos y flujos</p>
        </div>
        <button
          @click="openNewFlowModal"
          class="px-6 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl hover:from-blue-700 hover:to-indigo-700 font-bold flex items-center shadow-lg shadow-blue-500/20 dark:shadow-blue-900/20 transition-all hover:-translate-y-0.5"
        >
      ...
      <div v-if="flows.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div 
          v-for="flow in flows" 
          :key="flow.id" 
          class="bg-white dark:bg-slate-800/80 backdrop-blur-sm rounded-2xl shadow-sm dark:shadow-lg hover:shadow-xl dark:hover:shadow-2xl hover:bg-slate-50 dark:hover:bg-slate-800 transition-all p-6 border border-slate-200 dark:border-white/5 group relative overflow-hidden flex flex-col"
        >
          <!-- Background accent -->
          ...
          <p class="text-slate-500 dark:text-slate-400 text-sm mb-6 line-clamp-2 h-10 flex-grow">
          ...
          <div class="flex space-x-3 pt-5 border-t border-slate-100 dark:border-white/5 relative z-10">
            <router-link 
              :to="`/flows/${flow.id}`"
              class="flex-1 text-center bg-blue-50 dark:bg-blue-600/10 text-blue-600 dark:text-blue-400 py-2.5 rounded-xl hover:bg-blue-600 hover:text-white transition-all font-bold text-sm border border-blue-100 dark:border-blue-600/20 uppercase tracking-wide"
            >
              Ver Detalles
            </router-link>
            <button
      ...
      <div v-else class="col-span-full flex flex-col items-center justify-center py-20 bg-white dark:bg-slate-800/30 rounded-3xl border-2 border-dashed border-slate-200 dark:border-slate-700/50 backdrop-blur-sm">
        ...
        <p class="text-slate-800 dark:text-white text-2xl font-bold mb-3">No hay flujos creados</p>
        <p class="text-slate-500 dark:text-slate-400 max-w-md text-center">Crea tu primer flujo de trabajo para comenzar a organizar y monitorear tus tareas de manera eficiente.</p>
        <button
          @click="openNewFlowModal"
